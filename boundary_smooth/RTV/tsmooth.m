function [S, Wts1, Wts2] = tsmooth(I,lambda,sigma,sharpness,maxIter)
...
% 
    ...
    dec=2.0;
    for iter = 1:maxIter
        [wx, wy, wtb_inv, ret_inv] = computeTextureWeights(x, sigma_iter, sharpness);
...
function [retx, rety, wtb_inv, ret_inv] = computeTextureWeights(fin, sigma,sharpness)
   ...
   wtbx_inv = 1./wtbx;
   wtby_inv = 1./wtby;
   wtb_inv  = wtbx_inv + wtby_inv;
   retx_inv = 1./retx;
   rety_inv = 1./rety;
   ret_inv  = retx_inv + rety_inv;
   %figure(100), imshow([wtb_inv, ret_inv]);
   retx(:,end) = 0;
   rety(end,:) = 0;
end
