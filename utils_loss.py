...
def softmask_generator(shadow, shadow_free):
    fake_A2B_ = (shadow_free+1.)/2.                                                              #[1, 3, 256, 256], from (-1)-1 to 0-1
    real_A_   = (shadow+1.)/2.
    diffrAfB  = torch.mean((fake_A2B_-real_A_), dim=1, keepdim=True)                             #[1, 1, 400, 400]
    diffrAfB[diffrAfB<0.05]=0
    mask1crAfB = (diffrAfB - torch.min(diffrAfB)) / (torch.max(diffrAfB) - torch.min(diffrAfB))  #   0-1 #[1, 1, 400, 400]
    mask1crAfB = mask1crAfB*2-1                                                                  #(-1)-1 #[1, 1, 400, 400]
